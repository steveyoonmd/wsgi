...
def main():
    ...
    for k in [sys.argv[1], sys.argv[2]]:
        ...
        with open(os.path.join(data_dir, '{0}_{1}_{2}_{3}.{4}'.format('create', 'tbl', k, now, 'sql')), 'a') as tbl:
            tbl.write(';\n\n'.join(create_tables))
            tbl.write(';')

        # with open(os.path.join(data_dir, '{0}_{1}_{2}_{3}.{4}'.format('select', 'cnt', k, now, 'csv')), 'a') as cnt:
